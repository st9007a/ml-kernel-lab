...
def moe_grouped_expert_gemm_fwd_v3_fused_kernel(
    ...
    for expert_id in tl.range(N_EXPERTS):
        expert_start = tl.load(expert_offsets_ptr + expert_id)
        expert_end = tl.load(expert_offsets_ptr + expert_id + 1)

        num_m_tiles = tl.cdiv(expert_end - expert_start, BLOCK_M)
        expert_tile_end = expert_tile_start + num_m_tiles * num_n_tiles

        while tile_id >= expert_tile_start and tile_id < expert_tile_end:
            ...
            tile_id += step
        expert_tile_start = expert_tile_end
