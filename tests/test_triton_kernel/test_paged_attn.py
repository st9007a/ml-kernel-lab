...
def torch_decode_attention_gqa(q, k, v, seq_lens):
    outputs = []
    for b in range(q.shape[0]):
        seq_len = int(seq_lens[b].item())
        out = F.scaled_dot_product_attention(
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason='CUDA required')
@pytest.mark.parametrize(
    ('batch_size', 'n_query_heads', 'n_kv_heads', 'max_seq_len', 'head_dim', 'block_size', 'seq_lens_values'),
    [
        (1, 8, 2, 64, 64, 16, [64]),
        (2, 16, 4, 77, 64, 16, [77, 53]),
        (3, 8, 2, 129, 128, 32, [129, 97, 1]),
    ],
)
@pytest.mark.parametrize('dtype', [torch.float16, torch.bfloat16])
@pytest.mark.parametrize('permute_blocks', [False, True])
def test_v1_match_torch_gqa_results(
    batch_size,
    n_query_heads,
    n_kv_heads,
    max_seq_len,
    ...
    q = torch.randn((batch_size, n_query_heads, head_dim), dtype=dtype, device='cuda')
    k = torch.randn((batch_size, n_kv_heads, max_seq_len, head_dim), dtype=dtype, device='cuda')
    v = torch.randn((batch_size, n_kv_heads, max_seq_len, head_dim), dtype=dtype, device='cuda')
    seq_lens = torch.tensor(seq_lens_values, dtype=torch.int32, device='cuda')
    k_cache, v_cache, block_table, seq_lens = make_paged_cache(k, v, seq_lens, block_size, permute_blocks)
    max_num_blocks = (max(seq_lens_values) + block_size - 1) // block_size

    actual = triton_kernel.single_query_paged_kv_attention(q, k_cache, v_cache, block_table, seq_lens, max_num_blocks)
    expected = torch_decode_attention_gqa(q, k, v, seq_lens)
